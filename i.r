...
if(1 <= level || level <= 0) stop("Error: 'level' must be between '0' and '1'.")
if(length(sample) < 1e3) warning("Insufficient sample to produce reliable 'interval' estimates.")  
  sorted <- sort(sample)
...
for(i in 1:loop){
 ...
 return(round(data.frame(estimate = estimate, mean = mean, mode = mode, median = median, sd = sd, CI = CI, prob.eq = BB, BF01 = BF01, BF10 = BF10), 6))                                                
}     
