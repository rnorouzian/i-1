...
prop.priors <- function(a, b, lo = 0, hi = 1, dist.name, yes = 55, n = 1e2, scale = .1, top = 1.5, show.prior = FALSE){
  ...
  if(!pr){  
    ...
    plot(CI[, 1:2], rep(1:loop, 2), ty = "n", xlim = 0:1, ylim = c(1, top*loop), ylab = NA, yaxt = "n", xaxt = "n", xlab = "Credible Interval 'B'", font.lab = 2)
    ...
    axis(2, at = 1:loop, lab = substring(d, 2), font = 2, las = 1, cex.axis = .8, tck = -.006)
    
    for(i in 1:loop){
      polygon(x = h[[i]]$x, y = scale*h[[i]]$y + i, col = adjustcolor(i, .3), border = NA, xpd = NA)
    }
